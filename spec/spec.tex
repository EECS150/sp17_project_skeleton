...
\verb|i2c_controller.v| takes the same 50MHz clock and reset as the CPU. \verb|i2c_divider| takes in a value for internal clock division; we will set it \verb|0x01F4| to get a 100KHz data rate. 

This slave device, register address, read data, and write data are all normally 8 bits. However, in the provided controller, these register has 16 bits each in order to contain extra information. 
The slave address register takes the address in the top 15 bits, and an active high write enable in the LSB. 
The register address register and write data register take the address and data in the lower 8 bits, and the upper 8 bits are reserved for the write delay. The write delay sets an internal counter to delay the transaction, pending a response from an I2C device. 

\begin{table}[hbt]
	\begin{center}
		\caption{I2C Controller Input Mapping}
		\label{i2c_ctrl}
		\begin{tabular}{l l l l}
			\toprule
			\textbf{Register} & \textbf{Function} & \textbf{Data Encoding}\\
			\midrule
			i2c\_divider & Clock Divider Setting & \{divisor\} \\
			i2c\_slave\_addr\_unreg & Slave Device Address &  \{7'b0, slave address, active-low write enable\} \\
			i2c\_reg\_addr\_unreg & Slave Register Address & \{write delay, register address\} \\
			i2c\_wdata\_unreg & Register Write Data & \{8'h01, register data\} \\
		\end{tabular}
	\end{center}
\end{table}

The software will be responsible for formatting the data as such. As with the UART, the CPU simply needs access to the registers, as described in the next section. This information is provided to help with testbenching.

\subsubsection{Hookup I2C to User I/O}
...
\begin{table}[hbt]
	\begin{center}
		...
		\begin{tabular}{l l l l}
			...
			32'h80000114 & I2C register write trigger & Write & Executes I2C transaction \\
			%\midrule
		\end{tabular}
	\end{center}
\end{table}

Each port of the I2C controller has its own memory mapped register. This will operate in a very similar to the CPU-UART interface. The major different is in the trigger register. The trigger register is responsible for starting an I2C transaction. All of the I2C registers can be written to, but only writes to this register initiates the transfer.
% This tells the I2C controller that the data is valid, initializing the transaction.

Modify \verb|Riscv151.v| to include these memory mapped registers, as you did with the UART.
...
To help you test, we have provided an I2C slave model. This model will print out information about the transaction times, slave address, device register, register data, transaction acknowledgement, etc. This will allow you to check your memory map and some timing operations.

The provided I2C testbench attempts to manually read from and write to the I2C controller using the memory map. It can be found in \verb|software/i2c_integration_tb/|. Compile and run the program just as you did for the other testbenchs. This testbench allows you to check for any outstanding errors.



\subsection{Frame Buffer}
We will need a place store all of the pixel data for the image, a total of 786432 pixels. We will generate a 786432x1 block RAM, where each byte line stores a single pixel's data. Note that we will break from the previous convention of word alignment. To simplify the design, we will keep the RAM byte-aligned. Each byte will be either high or low, representing a light or dark pixel. The pixels will be ordered left to right, then top to bottom. This matches the VGA pixel order.

The CPU will be able to write to the frame buffer in the same manner it does the DMEM. Thus we need to reserve an address space for it. To make space for all 786432 pixels, we will use reserve the spaces 0x90000000 through 0x900BFFFF for the memory.
...
The Chrontel CH7301C DVI Transmitter Device on our FPGA is a simple interface to output DVI display data. It includes bypass controls to allow VGA output, which is what we will use.

\begin{figure}[hbt]
\begin{center}
	\includegraphics[width=6.5in]{dvi_block}
	\caption{Chrontel Transmitter Block Diagram}
	\label{fig:dvi_block}
\end{center}
\end{figure}
...
There are sequence of registers we need to set to activate the DVI device and begin transmission. These registers will be written over I2C. Below is a reduced list of our key registers. See the Chrontel datasheet for more detail. 
...
These registers will be set via the I2C software. The slave device address for the Chrontel device is 0x76.

\subsubsection{Hookup}

As with the AC97 device, we have to add the DVI device to the top module. Be sure the following are added to your top module and the UCF. Otherwise, run \verb|git pull staff master|. 

v


\subsection{VGA}
Since we are using the VGA bypass on the Chrontel device, we have to generate the necessary signals for VGA. Thus we must understand the VGA protocol.

\subsubsection{Timing}

\begin{figure}[hbt]
\begin{center}
	\includegraphics[width=4in]{vga_overview}
...
For each sync signals, there are 4 major components: the sync pulse, the back porch immediately following, the display active time, and the front porch before the next sync signal. Only during the display active time do we want to be sending valid data to be displayed.
...
We will be using 1024 x 768 at a 60 Hz refresh rate. This will require us to have 65 MHz clock and the following timing parameters.
...
\begin{table}[hbt]
	\begin{center}
		...
		\begin{tabular}{l l l l}
			...
			\midrule
				  Sync Pulse    &  AE8 & LVCMOS33 \\
				  Back Porch    &   AM12 & LVCMOS33 \\
				  Visible Area    &   AK6 & LVCMOS33 \\
				  Front Porch    &   AM11 & LVCMOS33 \\
			\midrule
				  DVI\_DE        &        AE8 & LVCMOS33 \\
				  DVI\_H         &        AM12 & LVCMOS33 \\
				  DVI\_RESET\_B  &         AK6 & LVCMOS33 \\
				  DVI\_V         &        AM11 & LVCMOS33 \\
		\end{tabular}
	\end{center}
\end{table}

We will be using one-bit color: either black or "white". We will tint our "white" a slight blue. This implementation is fairly simple expand to 3-color RGB and to multi-bit RGB, with 5-bits of color each. This is not a requirement of the project, but you are welcome to try it for extra credit.

\subsection{DVI Testbench}
...
\begin{center}
	\begin{tabular}{m{30mm} m{35mm} m{70mm}}
		...
		\textbf{Deliverable} & \textbf{Due Date} & \textbf{Description} \\
		\midrule
		I2C memory map + VGA controller + line drawing accelerator & \audioDueDate - Checkoff time in the evening TBA & Demonstrate a working VGA color bar test. Demonstrate line or image drawing from both the CPU and the accelerator \\
		\bottomrule
	\end{tabular}
\end{center}

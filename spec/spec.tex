...
\begin{table}[H]
	\begin{center}
		...
		\begin{tabular}{l l l l}
			...
			8'd1D & Input Clock Register & 8'h00 & 8'h0F \\
			8'h1F & Input Data Format & 8'h00 & 8'h83 
		\end{tabular}
	\end{center}
\end{table}

These registers will be set via the \verb|graphics| program; it is all done for you. The slave device address for the Chrontel device is 0x76.
...
All these signals are driven using the \verb|dvi_controller| located in \verb|src/video/dvi_controller.v|

% specify the source file dvi_controller.v
...
You will note that the 12-bit data input changes on both rising and falling clock edges. To support this, we will be buffer the rising phase data and the falling phase data with an ODDR buffer primitive. This buffer will update on both clock edges and can take two different inputs.



\subsubsection{Hookup}

As with the AC97 controller, we have to add the DVI controller to \verb|ml505top|. Be sure the following are added to your top module and the UCF. Otherwise, run \verb|git pull staff master|.

\subsection{DVI Testbench}

Your task is to build a controller that reads from the frame buffer and provides all the necessary controls for the Chrontel device. This includes the color data, the vertical and horizontal VGA sync signals, the differential XCLK as shown in the Chrontel data timing diagram, and the enable signal. 

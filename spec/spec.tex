...
\begin{itemize}
	\item \textbf{\labFiveDueDate} - Finish Lab 5 (UART) and begin Lab 6 (AC97 Controller).
...
Here are some commands to add a modules from a previous lab to your git repository. Make sure to first change into the \verb|hardware/src| directory of the skeleton files in your repo.
...
To only \textbf{synthesize} your design, go to the \verb|hardware/| directory and run \verb|make synth|. To \textbf{build} your entire design run \verb|make|. To \textbf{program} the FPGA, run \verb|make impact| from this same directory. To \textbf{simulate} your design, go to the \verb|hardware/| directory and run \verb|make sim|.
...
\begin{itemize}
	...
	\item \verb|UART.v, UATransmit.v, UAReceive.v|: Your working solution from Lab 5
	\item \verb|RegFile.v|: Your register file implementation
	...
	\item \verb|EchoTestbench.v|: Basic testbench for your CPU. Use this as an example and create others. It implements the echo FSM from lab 5 using software executed by your processor.
\end{itemize}
...
\begin{itemize}
 ...
 \item \verb|echo|: This directory contains the software necessary to run the echo program, which behaves exactly like in Lab 5.
 \item \verb|assembly_tests|: Use this as a template to write assembly tests for your processor designed to run in simulation.
 \item \verb|asmtest|: Use this as a template to write assembly tests for your processor to execute on the FPGA.
 \item \verb|example|: Use this as an example to write C programs.
...
Inside of \verb|hardware/src/imem_blk_ram|, \verb|hardware/src/dmem_blk_ram|, and \verb|hardware/src/bios_mem| there are three skeleton files:
\begin{itemize}
	\item \verb|*mem_blk_ram.xco|: This file contains configuration information
	...
	\item \verb|build|: Running \verb|./build| generates the memory based on the
	configuration information. Run this if you change the parameters in the 	\verb|.xco| file. You must run this if you decide to use a different \verb|.coe| file.
...
To make the block RAMs work in simulation, build them using any \verb|.coe| file you desire. \textbf{However, in simulation, the initial contents of the memory aren't specified by the .coe file, but rather a .mif file.} A \verb|.mif| file is generated by running \verb|make| in any software directory and it contains effectively the same data as the \verb|.coe| file.

In simulation, the memories are initialized with a \verb|.mif| file. The software toolchain generates these for you; look at the \verb|hardware/sim/test/*.do| files for examples of how to use these.
...
However, the memory address that the processor computes is a {\bf byte address}. This means that every 32 bit address the processor computes corresponds to one 8-bit space of memory.
...
These block RAMs have been provided for you in \verb|/hardware/src/imem_blk_ram|, \verb|/hardware/src/dmem_blk_ram|, and \verb|/hardware/src/bios_mem|. See Section \ref{ram_info} for details about initializing and using the block RAMs. To simplify things for later, you are \textbf{required} to instantiate these block RAMs in the top level of \verb|Riscv151.v|.
...
For example, a store to an address beginning with \verb|0x3| will write to both the instruction memory and data memory, while storing to addresses beginning with \verb|0x2| or \verb|0x1| will write to only the instruction or data
memory, respectively. For details about the BIOS and how to run programs on your CPU, see Section~\ref{bios_info}.
...
At this stage in the project the only way to interact with your CPU is through
the serial interface. The UART from Lab 5 accomplishes the low-level task of
...
\begin{enumerate}
	...
	\item Test the entire CPU one instruction at a time with an assembly program
	\item Test serial I/O --- see the provided file \verb|EchoTestbench.v|
\end{enumerate}
...
To run new simulations, you should write new testbenches and put them
in your \verb|hardware/src| directory. Then you should create new
\verb|.do| files in the \verb|hardware/sim/tests| directory. The staff
have provided you with \verb|EchoTestbench.v| and the corresponding \verb|.do| file. When you run \verb|make sim| in the \verb|hardware| directory, all of the \verb|.do| files in the \verb|sim/tests| directory will run. If you don't want to run some of the tests, then just change the file extension to something other than \verb|.do|.

Just like in the labs, you can debug your logic by looking at waveforms. After running your simulations with \verb|make sim|, run the \verb|viewwave| script in the \verb|hardware/sim| directory, passing in the \verb|.wlf| file as an argument:
...
In \verb|echo.do| you will see lines that look like this:
...
Once you are confident that the individual components of your processor are working in isolation, you will want to test the entire processor as a whole. The easiest way to do this is to write an assembly program that tests all of the instructions in your ISA. A skeleton is provided for you in \verb|software/src/assembly_tests|. See Section \ref{testing} for details.
...
Take a look at the \verb|software/example| folder for an example of a C program.

There are several files in the example project, each with a specific purpose:
\begin{itemize}
    \item \verb|start.s|: This is an assembly file that contains the start of the program. It initializes the stack pointer then jumps to the \verb|main| label. Edit this file to move the top of the stack. You will have to move the top of the stack to give it some room to grow.
    \item \verb|example.ld|: This linker script sets the base address of the program. For checkpoint 1, this address should be in the format \verb|0x1000xxxx|
    \item \verb|example.elf|: Binary produced after running \verb|make|. Use \verb|riscv-objdump -D example.elf| to view the assembly code. You may want to redirect the output to a file or pipe it through less to examine the assembly.
    \item \verb|example.mif|: Produced by the toolchain. Use this to initialize the block RAMs in ModelSim.
    \item \verb|example.coe|: Produced by the toolchain. Use this to initialize the block RAMs during generation (coregen).
\end{itemize}

\subsection{Assembly Tests}

This section describes the contents of \verb|software/src/assembly_tests|. You can test individual instructions in simulation with a program similar to the following example in \verb|assembly_tests/start.s|:
...
\begin{enumerate}
	...
	\item Rebuild your instruction, data, and BIOS memories with the generated \verb|.coe| file by copying the \verb|.coe| file into the \verb|hardware/src/{imem, dmem}_blk_ram| and \verb|hardware/src/bios_mem| directories, and then re-running the \verb|build| script
	\item Build your CPU and impact it to the board
	\item Press the CPU\_RESET button to reset your CPU
	\item As in lab 6, use screen to access the serial port:
	\begin{minted}[tabsize=2]{bash}
	screen $SERIALTTY 115200
	\end{minted}
\end{enumerate}

\textbf{Please remember to shut down screen using Ctrl-a shift-k, or other
students won't be able to use the serial port!}
...
In addition to the command interface, the bios also allows you to load programs
to the CPU\@. Close screen using ctrl-a shift-k, and execute in the terminal:
\begin{minted}[tabsize=2]{bash}
    coe_to_serial <coe_file> <address>
\end{minted}

This stores the \verb|.coe| file at the specified hexadecimal address. In order to write into both the data and instruction memories, remember to set the top nibble to 0x3 (i.e. \verb|coe_to_serial echo.coe 30000000|, assuming the \verb|.ld| file sets the base address to 0x10000000). You also
need to ensure that the stack and base address are set properly (See Section \ref{soft_append}).
...
\textbf{\baseCPUTaskName \space materials should be committed to your project repository by \dueDateTime \space \baseCPUDueDate.} The in person checkoff will be performed the following Friday (November 18th) due to Veteran's Day.
...
We will then add the \verb|tone_generator| created in lab to our design as a peripheral and allow programs to access its \verb|tone_switch_period| and \verb|output_enable| inputs over memory mapped I/O. You will be able use your processor to simulate the \verb|music_streamer| FSM built in lab using software.
...
\subsubsection{Building a Synchronous FIFO}
...
\subsubsection*{FIFO Functionality}
...
\subsubsection*{FIFO Interface}
Take a look at the FIFO skeleton in \verb|hardware/src/fifo.v|.
...
\begin{itemize}
	...
	\item Rotary wheel spinning and the rotary pushbutton
...
Copy over your \verb|tone_generator.v| from Lab 4 to the \verb|hardware/src/| directory. Recall that your \verb|tone_generator| takes a \verb|tone_switch_period| which describes how many clock cycles the \verb|tone_generator| takes to invert its \verb|square_wave_out| output. There is also an \verb|output_enable| input into the \verb|tone_generator| which gates the \verb|square_wave_out| output low.
...
\begin{itemize}
	\item \verb|on| - Flips high the output enable register
	\item \verb|off| - Flips low the output enable register
...
The music streamer program can be found in \verb|software/music_streamer|. To use this program, use the same scripts from Lab 4 to generate a music data file from a MusicXML file, and then convert that data file to a static array declaration that can be used in a C program.
...
Now, we have a file called \verb|music.h| that has a static array declaration filled with the music data. This serves exactly the same purpose as the ROM that was generated in Lab 4.
...
The \verb|music_streamer| functions exactly like it does in Lab 4, but the state machine that was implemented directly in hardware, is now implemented in software. Here is the state machine diagram:
...
\subsection{Asynchronous FIFOs, Multi-bit Clock Crossing}
...
\subsubsection{Async FIFO Construction}
...
\begin{itemize}
	\item Use one synchronous reset signal with respect to one clock domain, and propagate the reset to the other clock domain internally to the FIFO with a 1-bit synchronizer.
...
For this project, we are going to go with the last option as it is the most robust for our target (an FPGA and a temperamental AC97 bit clock). This means that you should initialize any \verb|reg| nets that represent actual registers in your async FIFO design.

\subsubsection{Async FIFO Interface}
...
\subsubsection{Async FIFO Timing}
...
\subsubsection{Async FIFO Testing}
...
\subsubsection{AC97 Controller Testbench}
We have provided a testbench for the AC97 controller in \verb|hardware/src/AC97Testbench.v|. The testbench tests your AC97 controller in isolation and verifies that it sets the master volume, headphone volume, and PCM-out volume registers correctly, and verifies the PCM data sent in slots 3 and 4. 

\textbf{It has been discovered that this testbench makes certain assumptions about how your controller fetches samples from the async FIFO, and thus may fail even if your controller is written properly.} Use the testbench below if you are having trouble debugging this testbench.

\subsubsection{AC97 Controller Integration Testbench}
An integration testbench has been provided in \verb|/hardware/src/ac97_integration_testbench.v|. The software for this testbench is in \verb|/software/ac97_integration_tb/ac97_integration_tb.c|. 
...
To run this testbench, you will have to modify the instantiations in the Verilog file to match your CPU's port declaration. You only need to connect the ports that your CPU needs to communicate with the async FIFO (\verb|wr_en|, \verb|full|, and \verb|din|) and the AC97 controller (\verb|volume_control|). Run this testbench as usual with \verb|make sim|.
...
We have provided a program in \verb|software/AC97/| that sends samples to the AC97 controller via the memory mapped I/O interface. Build and run this program just like \verb|mmult|. Plug in your headphones or speakers to the headphone jack on the board. This program sends a square wave of a certain frequency to the AC97 sample FIFO, and updates the volume register with the state of the DIP switches.
...
To test your AC97 controller, a program will be provided that will use your processor to generate sine waves and send them to the AC97 controller via the async FIFO memory map. This program is in \verb|/software/ac97_piano|. Compile and run this program in the same way as the previous ones.

Once you execute \verb|jal 10006000|, you should be able to use your keyboard as a piano and play notes by typing into screen. You will hear the output coming from the headphone jack. To switch octaves, hold down the shift key while playing keys. You may want to look at the keymap in \verb|ac97_piano.c| to figure out what characters map to what notes.
...
\begin{itemize}
	\item Branch Predictor: Implement a two bit (or more complicated) branch predictor to replace the naive 'always taken' predictor used in the project
	\item 5-Stage Pipeline: Add more pipeline stages and push the clock frequency past 100MHz
	\item Audio Recording: Enable capturing mic input from the AC97 controller
...
1: RISCV151 Processor & Design Review &  \blockDiagramDueDate \space @ \dueDateTime\\
	 & Check in code to Github & \baseCPUDueDate \space @ \dueDateTime \\
	 & In-lab Checkoff & Friday, November 18 @ \dueDateTime\\
\midrule
2: IO, FIFOs, AC97 Controller & Check in code to Github & Friday, November 25\\
 & In-lab Checkoff & Friday, December 2 @ \dueDateTime \\
\midrule
\midrule
Final Checkoff, Extra Credit, &    In-lab Checkoff        & \finalCheckoffDueDate \space @ \dueDateTime \\
